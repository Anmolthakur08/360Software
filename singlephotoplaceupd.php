...
$user = $_SESSION['user'];
    ...
    if ($response === false) {
        $_SESSION['singlephotoplaceupderror'] = 'Curl error:';
    ...
    if (!isset($responseData['access_token'])) {
        $_SESSION['singlephotoplaceupderror'] = 'Error obtaining access token:';
        header("Location: g_storeimages.php?id=$storeId");
        exit();  
    }
...
if (!$storeId) {
    ...
    $_SESSION['singlephotoplaceupderror'] = 'Missingparameter in URL. Use ?id=123';
    header("Location: g_storeimages.php?id=$storeId");
    exit();  
}

// Fetch photo IDs and place ID
$stmt = $conn->prepare("SELECT `360photoId`, `360placeId` FROM 360g_storeimages WHERE storeID = ?");
...
while ($row = $result->fetch_assoc()) {
    ...
    if (!$placeId && !empty($row['360placeId'])) {
        $placeId = $row['360placeId'];
    }
}
$stmt->close();
if (empty($photoIds)) {
     $_SESSION['singlephotoplaceupderror'] = 'No photo IDs found for store ID';
    header("Location: g_storeimages.php?id=$storeId");
    exit();  
}
if (!$placeId) {
    $_SESSION['singlephotoplaceupderror'] = 'No place ID found for store ID First Time Publish All Image ';
    header("Location: g_storeimages.php?id=$storeId");
    exit();  
...
if (!isset($response['error'])) {
    ...
    $updateStmt = $conn->prepare("UPDATE 360g_storeimages SET 360status = 'published' WHERE storeID = ? AND 360photoId = ?");
    if (!$updateStmt) {
         $_SESSION['singlephotoplaceupderror'] = 'Prepare failed: ';
         header("Location: g_storeimages.php?id=$storeId");
         exit();  
    }

    foreach ($photoIds as $photoId) {
        $updateStmt->bind_param("is", $storeId, $photoId);
        $updateStmt->execute();
    }
    ...
    header("Location: create-connections.php?storeId=$storeId");
    exit;
} else {
    $_SESSION['singlephotoplaceupderror'] = 'An error occurred during batch update.';
    header("Location: g_storeimages.php?id=$storeId");
    exit();  
}